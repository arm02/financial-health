...
func (u *TransactionUseCase) GetAllTransaction(ctx context.Context, userID int64, page, limit int, sortBy, sortType, query string) (*domain.RowsList[domain.Transaction], error) {
	if page <= 0 {
		page = 1
	}
	if limit <= 0 {
	...
	transactions, total, err := u.trxRepo.GetAll(ctx, userID, page, limit, sortBy, sortType, query)
	if err != nil {
		return nil, err
	}
